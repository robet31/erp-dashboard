...
async function proxyToFrappe(request: NextRequest, pathSegments: string[], method: string) {
  ...
  try {
    ...
    if (response.status === 403) {
      const errorText = await response.text();
      return NextResponse.json(
        { error: 'Access Forbidden (403)', message: 'API Key/Secret salah', detail: errorText },
        { status: 403 }
      );
    }
    ...
    // TERUSKAN SEMUA ERROR (TERMASUK 417) KE FRONTEND AGAR KITA TAHU ALASANNYA
    if (!response.ok || data.exc || data.error || data._server_messages) {
    ...
    return NextResponse.json(
      { error: 'Failed to connect to ERPNext', message: String(error) },
      { status: 502 }
    );
  }
}