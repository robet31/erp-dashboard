...
import { ROLES, type UserRole } from '@/config/rbac';
import {
  Eye, EyeOff, ChevronDown, ChevronUp, CheckCircle, Trash2, Cog, Zap
} from 'lucide-react';
...
// Data Quick Login berdasarkan seed.ts
const QUICK_LOGINS = [
  { label: 'Administrator', email: 'admin@erp.com', password: 'password123', role: 'administrator', color: '#dc2626' },
  { label: 'Admin Sales', email: 'sales@erp.com', password: 'password123', role: 'admin_sales', color: '#2563eb' },
  { label: 'Admin Gudang', email: 'gudang@erp.com', password: 'password123', role: 'admin_gudang', color: '#16a34a' },
  { label: 'Man. Produksi', email: 'produksi@erp.com', password: 'password123', role: 'manajer_produksi', color: '#d97706' },
];
...
export default function LoginPage() {
  ...
  const handleQuickLogin = async (quickUser: typeof QUICK_LOGINS[0]) => {
    setIsLoading(true); setError('');
    try {
      await login(quickUser.role as UserRole, quickUser.email, quickUser.password);
      router.push(getRedirectPath(quickUser.role));
    } catch (error: any) {
      setError(error.message || `Gagal login sebagai ${quickUser.label}`);
    } finally { setIsLoading(false); }
  };
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── QUICK LOGIN ── */
        .lp-divider {
          display: flex; align-items: center; text-align: center; margin: 24px 0 16px;
        }
        .lp-divider::before, .lp-divider::after {
          content: ''; flex: 1; border-bottom: 1px solid #e2e8f0;
        }
        .lp-divider span {
          padding: 0 12px; color: #94a3b8; font-size: 11px; font-weight: 600; letter-spacing: 0.05em;
        }
        .lp-quick-grid {
        ...
        .lp-quick-btn:hover {
          border-color: var(--btn-color); color: var(--btn-color); background: #f8fafc;
        }
        .lp-quick-btn:disabled { opacity: 0.5; cursor: not-allowed; }

        /* ── SWITCH ── */
        .lp-switch { text-align: center; font-size: 13.5px; color: #64748b; margin-top: 10px; }
        .lp-switch-lnk {
          font-weight: 700; cursor: pointer; margin-left: 4px;
          transition: color 0.2s;
          background: linear-gradient(135deg, #054CC7, #17C3CC);
        ...
        @media (max-width: 820px) {
          .lp-card {
            flex-direction: column; max-width: 420px; width: calc(100% - 32px);
            border-radius: 24px; height: auto; max-height: calc(100vh - 40px); overflow-y: auto;
          }
          .lp-left {
            width: 100%; padding: 18px 22px; flex-direction: row; align-items: center; gap: 14px; flex-shrink: 0;
          }
          .lp-left-decor, .lp-left-accent { display: none; }
          .lp-logo { margin-bottom: 0; flex-shrink: 0; }
          .lp-slogan, .lp-illus-wrap { display: none; }
          .lp-right { padding: 24px 26px 28px; overflow-y: visible; }
          .lp-heading { font-size: 22px; margin-bottom: 4px; }
          .lp-subheading { margin-bottom: 20px; }
          .lp-field-group { margin-bottom: 12px; }
        }
        ...
        @media (max-width: 480px) {
          .lp-root { align-items: center; padding: 16px; }
          .lp-card { max-width: 100%; width: 100%; border-radius: 20px; height: auto; max-height: calc(100vh - 32px); overflow-y: auto; }
          .lp-left { padding: 14px 18px; border-radius: 20px 20px 0 0; }
          .lp-right { padding: 20px 18px 24px; }
          .lp-heading { font-size: 20px; }
          .lp-subheading { font-size: 12px; margin-bottom: 16px; }
          .lp-field-group { padding: 10px 13px; margin-bottom: 10px; border-radius: 10px; }
          .lp-label { font-size: 9px; margin-bottom: 3px; }
          .lp-input { font-size: 13px; }
          .lp-submit { padding: 13px; font-size: 14px; border-radius: 10px; }
          .lp-extras { margin: 4px 0 13px; }
          ...
          .lp-hard-reset { font-size: 11px; margin-top: 12px; }
          .lp-error { font-size: 12px; padding: 9px 13px; margin-bottom: 12px; }
          .lp-quick-grid { grid-template-columns: 1fr; gap: 6px; }
        }
        ...
        <div className="lp-card">
          ...
          <div className="lp-right">
            <div className="lp-right-content">
              ...
              {/* SECTION QUICK LOGIN - Hanya tampil saat mode login */}
              {mode === 'login' && (
                <>
                  <div className="lp-divider"><span>ATAU QUICK LOGIN</span></div>
                  <div className="lp-quick-grid">
                    {QUICK_LOGINS.map((qUser, idx) => (
                      <button
                        key={idx}
                        type="button"
                        ...
                        disabled={isLoading}
                        style={{ '--btn-color': qUser.color } as React.CSSProperties}
                      >
                        <Zap size={14} color={qUser.color} />
                        {qUser.label}
                      </button>
                    ))}
                  </div>
