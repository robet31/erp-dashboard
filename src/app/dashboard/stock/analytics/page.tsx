...
import { Loader2, Info, X, Package, Warehouse, DollarSign } from 'lucide-react';
import {
...
export default function StockAnalyticsPage() {
  // FIX: Mengambil `warehouses` langsung dari Frappe API (useStockData)
  ...
  const stats = useMemo(() => {
    ...
    // Murni membaca jumlah Master Gudang
    ...
    // Inisialisasi semua gudang dengan nilai 0 (agar gudang kosong tetap muncul di chart)
    ...
    // Jangan filter (w => w.value > 0) jika ingin menampilkan semua gudang (termasuk yang 0)
    // Di sini saya asumsikan kita tampilkan semua gudang dari master
  ...
  if (isStockLoading || isSellingLoading) return <div style={{ textAlign: 'center', padding: '60px 20px' }}><Loader2 className="animate-spin" size={32} color={COLOR_PRIMARY} style={{ margin: '0 auto 16px' }} /><p style={{ color: '#6B7280', fontSize: '13px' }}>Memuat data Dashboard...</p></div>;
  ...
  const ChartHeader = ({ title, subtitle, infoText }: any) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
      <div>
        <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#111827' }}>{title}</h3>
        {subtitle && <p style={{ fontSize: '12px', color: '#6B7280', marginTop: '4px' }}>{subtitle}</p>}
      </div>
      <div>
        {infoText && (
          <button onClick={() => setInfoData({ show: true, title, text: infoText })} style={{ background: '#f3f4f6', border: 'none', borderRadius: '6px', padding: '4px 6px', cursor: 'pointer', color: '#9ca3af', display: 'flex', transition: 'color 0.2s' }} onMouseOver={e => e.currentTarget.style.color = COLOR_PRIMARY} onMouseOut={e => e.currentTarget.style.color = '#9ca3af'}>
            <Info size={16} />
          </button>
        )}
      </div>
    ...
    <div style={{ animation: 'fadeIn 0.4s ease-out', fontFamily: "'Inter', 'Poppins', sans-serif" }}>
      ...
      <div className="chart-container" style={{ marginBottom: '16px' }}>
        <ChartHeader 
          ...
          infoText="Grafik ini menampilkan total nilai valuasi uang (dalam Rupiah) dari seluruh barang yang tersimpan di masing-masing gudang (Termasuk gudang yang kosong/Rp 0)." 
        />
        {stats.whStockValue.length > 0 ? (
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={stats.whStockValue} margin={{ top: 20, right: 30, left: 10, bottom: 50 }}>
              ...
              <YAxis width={80} tickFormatter={(v) => formatShortAxis(v)} tick={{ fontSize: 11, fill: '#6B7280', fontFamily: 'Poppins' }} axisLine={false} tickLine={false} />
              <Tooltip content={<FrappeChartTooltip isCurrency={true} />} cursor={{ fill: '#f8fafc' }} />
              <Bar dataKey="value" name="Total Value" fill={TREND_COLOR_1} barSize={80} radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        ) : (
