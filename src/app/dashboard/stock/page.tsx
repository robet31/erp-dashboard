...
//                 <div className="tw-filter-selects-wrapper" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
...
//           .tw-filter-inputs-wrapper { flex-direction: column; align-items: stretch !important; gap: 10px; }
//           .tw-filter-inputs-wrapper > div:first-child { max-width: 100% !important; }
//           .tw-filter-selects-wrapper { width: 100%; flex-wrap: wrap; }
//           .tw-filter-selects-wrapper select { flex: 1 1 45%; }
//           .tw-filter-actions-wrapper { width: 100%; }
//           .tw-filter-actions-wrapper button { flex: 1; justify-content: center; }
//         }
//       `}</style>
//     </div>
//   );
...
import {
  Package, Warehouse, AlertTriangle, Filter,
  Plus, Search, X, Edit, Trash2, AlertCircle, Eye, Send, Info, CheckCircle, Loader2, Link as LinkIcon
} from 'lucide-react';
import { formatNumber, formatDate } from '@/lib/utils';
import { TableSkeleton } from '@/components/EmptyState';
...
function CreateDeliveryNoteModal({ onClose, customers, items, orders, warehouses, bins, onSuccess, showToast }: any) {
  const defaultCompany = useMemo(() => getDynamicCompany(warehouses), [warehouses]);
  const [form, setForm] = useState({ 
    customer: '', company: defaultCompany, posting_date: new Date().toISOString().split('T')[0], 
    is_return: false, items: [] as any[], linked_so: '' 
  });
  
  const [itemForm, setItemForm] = useState({ item_code: '', qty: '', rate: '', amount: 0, warehouse: '' });
  const [editingItemIndex, setEditingItemIndex] = useState<number | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const activeWarehouses = useMemo(() => warehouses.filter((w: any) => !w.is_group), [warehouses]);
  ...
  const isStockShort = !form.is_return && (Number(itemForm.qty || 0) > availableStock);

  // Di dalam CreateDeliveryNoteModal, perbarui handleSOChange:
  ...
  try {
    ...
    if (data.data) {
      ...
      setForm(f => ({
        ...
        items: soData.items.map((i: any) => ({ 
          item_code: i.item_code, 
          item_name: i.item_name, 
          qty: i.qty, 
          rate: i.rate, 
          amount: i.amount, 
          warehouse: i.warehouse || '', // Ambil gudang jika sudah ada di SO
          against_sales_order: soName,
          so_detail: i.name // <--- INI KUNCINYA: ID unik baris item di Sales Order
        }))
      }));
      showToast('Data ditarik! Silakan klik Edit (✏️) pada barang untuk menentukan gudang.', 'info');
    }
  } catch (err) {
    showToast('Gagal menarik rincian baris Sales Order', 'error');
  }
};

// Pastikan fungsi handleAddOrUpdateItem juga membawa properti so_detail:
const handleAddOrUpdateItem = () => {
  if (!itemForm.item_code || !itemForm.warehouse || Number(itemForm.qty) <= 0) {
    return showToast('Lengkapi Item, Gudang, dan isi Qty > 0', 'error');
  }

  const selectedItem = items.find((i: any) => i.item_code === itemForm.item_code);
  
  // Ambil data so_detail dari item lama jika dalam mode edit
  const existingItem = editingItemIndex !== null ? form.items[editingItemIndex] : null;

  const newItemObj = { 
    item_code: itemForm.item_code, 
    item_name: selectedItem?.item_name || itemForm.item_code, 
    qty: parseFloat(itemForm.qty), 
    rate: parseFloat(itemForm.rate || '0'), 
    amount: Number(itemForm.qty) * Number(itemForm.rate || 0), 
    warehouse: itemForm.warehouse, 
    against_sales_order: form.linked_so,
    so_detail: existingItem?.so_detail || null // Tetap bawa ID barisnya
  };

  if (editingItemIndex !== null) {
    const updatedItems = [...form.items];
    updatedItems[editingItemIndex] = newItemObj;
  ...
  setItemForm({ item_code: '', qty: '', rate: '', amount: 0, warehouse: '' });
};
  ...
  const handleEditItemClick = (index: number) => {
    ...
    setEditingItemIndex(index); // Simpan index yang sedang diedit
    // Fokus akan otomatis berpindah ke dropdown Gudang/Item karena data sudah ter-binding
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); 
    if (form.items.length === 0) return showToast("Daftar barang masih kosong!", 'error');
    if (form.items.some(i => !i.warehouse)) return showToast("Ada barang yang belum dipilih Gudang asalnya!", 'error');

    setIsSubmitting(true);
    try {
      const dnData = { 
        customer: form.customer, posting_date: form.posting_date, is_return: form.is_return ? 1 : 0, 
        company: defaultCompany, items: form.items 
      };
      const { apiCreate } = await import('@/lib/api');
      await apiCreate('Delivery Note', dnData);
      showToast('Surat Jalan berhasil dibuat!', 'success'); onClose(); if (onSuccess) onSuccess();
    } catch (err: any) { showToast(extractFrappeError(err), 'error'); } finally { setIsSubmitting(false); }
  };
  ...
  return (
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget && !isSubmitting) onClose(); }}>
      <div className="modal-content" style={{ width: '100%', maxWidth: '800px', margin: '0 16px', padding: 0, overflow: 'hidden' }}>
        <div style={{ background: '#f8f9fb', padding: '16px 24px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2 style={{ fontSize: '18px', fontWeight: 800, color: '#111827', margin: 0 }}>Buat Surat Jalan</h2>
          </div>
          <button onClick={onClose} disabled={isSubmitting} style={{ background: 'none', border: 'none', cursor: 'pointer' }}><X size={20} /></button>
        </div>

        <form onSubmit={handleSubmit} style={{ padding: '24px', maxHeight: '75vh', overflowY: 'auto' }}>
          {/* Section Sales Order Link */}
          <div style={{ background: '#eff6ff', padding: '12px', borderRadius: '10px', border: '1px dashed #bfdbfe', marginBottom: '20px' }}>
            <label className="erp-label" style={{ color: COLOR_PRIMARY, display: 'flex', alignItems: 'center', gap: '6px' }}>
              <LinkIcon size={14}/> Tarik Data dari Sales Order
            </label>
            <select className="erp-input" value={form.linked_so} onChange={handleSOChange} disabled={form.items.length > 0}>
              <option value="">-- Pilih Sales Order --</option>
              {orders && orders.map((o: any) => <option key={o.name} value={o.name}>{o.name} - {o.customer_name}</option>)}
            </select>
          </div>

          {/* Form Input Item (Digunakan untuk Tambah & Update) */}
          <div style={{ background: editingItemIndex !== null ? '#fffbeb' : '#f8fafc', padding: '16px', borderRadius: '12px', border: `1px solid ${editingItemIndex !== null ? COLOR_SECONDARY : '#e2e8f0'}`, marginBottom: '20px' }}>
            <h4 style={{ fontSize: '13px', fontWeight: 800, marginBottom: '12px', color: editingItemIndex !== null ? '#b45309' : '#475569' }}>
               {editingItemIndex !== null ? `✏️ Sedang Mengedit Baris Ke-${editingItemIndex + 1}` : '➕ Tambah Barang Manual'}
            </h4>
            <div className="responsive-grid" style={{ marginBottom: '12px' }}>
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label className="erp-label">Gudang Asal *</label>
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <select className="erp-input" value={itemForm.item_code} onChange={e => {
                  const val = e.target.value;
                  const sel = items.find((i: any) => i.item_code === val);
                  setItemForm(f => ({ ...f, item_code: val, rate: String(sel?.standard_rate || 0), amount: Number(f.qty) * (sel?.standard_rate || 0) }));
                }}>
                  <option value="">Pilih Item...</option>
                  {items.map((i: any) => <option key={i.name} value={i.item_code}>{i.item_code}</option>)}
                </select>
              </div>
            </div>
            
            <div className="responsive-grid-3" style={{ alignItems: 'flex-end' }}>
              <div className="form-group" style={{ marginBottom: 0 }}><label className="erp-label">Qty</label><input type="number" className="erp-input" value={itemForm.qty} onChange={e => setItemForm(f => ({ ...f, qty: e.target.value }))} /></div>
              <div className="form-group" style={{ marginBottom: 0 }}><label className="erp-label">Rate (Rp)</label><input type="number" className="erp-input" value={itemForm.rate} onChange={e => setItemForm(f => ({ ...f, rate: e.target.value }))} /></div>
              
              {/* Tombol berubah sesuai status edit */}
              <button type="button" onClick={handleAddOrUpdateItem} className="btn btn-primary" style={{ height: '42px', width: '100%', background: editingItemIndex !== null ? COLOR_SECONDARY : COLOR_PRIMARY, borderColor: editingItemIndex !== null ? COLOR_SECONDARY : COLOR_PRIMARY }}>
                {editingItemIndex !== null ? '✓ Update Baris' : '＋ Tambah'}
              </button>
            </div>
          </div>
          ...
          {form.items.length > 0 && (
            <div style={{ overflowX: 'auto', border: '1px solid #e5e7eb', borderRadius: '8px' }}>
              <table className="erp-table" style={{ width: '100%', margin: 0 }}>
                <thead style={{ background: '#f8fafc' }}><tr><th>No.</th><th>Item & Gudang</th><th style={{ textAlign: 'right' }}>Qty</th><th style={{ textAlign: 'right' }}>Amount</th><th style={{ textAlign: 'center' }}>Aksi</th></tr></thead>
                <tbody>
                  {form.items.map((item: any, i: number) => (
                    <tr key={i} style={{ background: editingItemIndex === i ? '#fffbeb' : 'transparent' }}>
                      <td>{i + 1}</td>
                      <td>
                        <div style={{ fontWeight: 700 }}>{item.item_code}</div>
                        {item.warehouse ? <div style={{ color: '#059669', fontSize: '11px' }}>🏠 {item.warehouse}</div> : <div style={{ color: '#ef4444', fontSize: '11px', fontWeight: 800 }}>⚠️ Gudang Belum Dipilih!</div>}
                      </td>
                      <td style={{ textAlign: 'right' }}>{item.qty}</td>
                      <td style={{ textAlign: 'right' }}>{formatUang(item.amount)}</td>
                      <td style={{ textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                          <button type="button" onClick={() => handleEditItemClick(i)} title="Edit Baris Ini" style={{ background: '#e0f2fe', color: COLOR_PRIMARY, border: 'none', padding: '6px', borderRadius: '6px', cursor: 'pointer' }}><Edit size={14}/></button>
                          <button type="button" onClick={() => handleRemoveItem(i)} title="Hapus" style={{ background: '#fee2e2', color: '#dc2626', border: 'none', padding: '6px', borderRadius: '6px', cursor: 'pointer' }}><Trash2 size={14}/></button>
                        </div>
                      </td>
                    </tr>
                  ))}
...
function StockPageContent() {
  const router = useRouter();
  const { items, warehouses, bins, stockEntries, isLoading, refetch } = useStockData();
  const { customers, deliveryNotes, salesOrders, isLoading: isSellingLoading, refetch: refetchSelling } = useSellingData();
  ...
  return (
    <div className="tw-root" style={{ fontFamily: "'Inter', 'Poppins', sans-serif", animation: 'fadeIn 0.4s ease-out' }}>
      ...
      {showCreateItemModal && <CreateItemModal onClose={() => setShowCreateItemModal(false)} onSuccess={() => refetch()} showToast={showToast} />}
      {showCreateWarehouseModal && <CreateWarehouseModal onClose={() => setShowCreateWarehouseModal(false)} onSuccess={() => refetch()} showToast={showToast} />}
      {showCreateDNModal && <CreateDeliveryNoteModal onClose={() => setShowCreateDNModal(false)} customers={customers} items={sortedItems} orders={salesOrders} warehouses={warehouses} bins={bins} onSuccess={() => refetchSelling()} showToast={showToast} />}
      ...
      <div className="tw-table-wrapper">
         <div className="tw-table-header">
            ...
            <div className="tw-table-tabs" style={{ overflowX: 'auto', whiteSpace: 'nowrap' }}>
               <button className={activeTab === 'items' ? 'active' : ''} onClick={() => setActiveTab('items')}>Items</button>
               <button className={activeTab === 'warehouse' ? 'active' : ''} onClick={() => setActiveTab('warehouse')}>Warehouses</button>
               <button className={activeTab === 'bin' ? 'active' : ''} onClick={() => setActiveTab('bin')}>Stock Level</button>
               <button className={activeTab === 'stockentry' ? 'active' : ''} onClick={() => setActiveTab('stockentry')}>Stock Entries</button>
